...
def driver(platform, browser):
    if platform is None or platform == "web":
        ...
        #command_executor_url = "http://192.168.1.100:4444"
        ...
        drvr = mobile_driver.Remote(
            command_executor_url='http://localhost:4723/wd/hub', options=options)
    else:
        print("Please enter valid value for --platform parameter")
        sys.exit()
    # driver = webdriver.Remote(ommand_executor=command_executor_url options=options)
